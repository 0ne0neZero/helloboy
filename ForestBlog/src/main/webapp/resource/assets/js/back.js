/******编辑*********/
//initView
function bj(id,name,author) {
	//console.log($(name).html());
	//console.log(author);
	//console.log(id);
	console.log($(".layui-upload-img").attr("src"));
	var doname,doauthor;
	...
	if(typeof author == "number") {
		...
		doauthor=$(author).html();
	}
	$("#zzc").show(200);
	//$(".layui-upload-img").attr("src");
	$("input[name='bookName']").val(doname);
	$("input[name='bookAuthor']").val(doauthor);
	$("#hiddenInput").data("bookId",id);
}
$("#qx").click(function() {
	$("#zzc").hide(200);
});
...
function save() {
	...
	var bookId = $("#hiddenInput").data("bookId");
	var imageUrl = $("input[name='userAvatar']").val();
	...
	$.ajax({
        type: "POST",
        data: param,
        url:'/admin/bookshelf/doUpdateObject',
...
function deleteBook(id) {
    if(confirmDelete()==true){
        ...
        $.ajax({
            async: false,
            type: "POST",
            url:'/admin/bookshelf/doDeleteObjectOne/'+id,
            contentType : "application/x-www-form-urlencoded; charset=utf-8",
            dataType: "text",
            complete:function () {
               window.location.reload();
...
function queryBook() {
  ...
  //提交form//
	$("#bookForm").attr("action", "/admin/bookshelf/search");
...
//批量删除书籍
function confirmDeleteBookBatch() {
    if(confirmDelete()==true){
        var text = $("input:checkbox[name='ids']:checked").map(function(index,elem) {
            return $(elem).val();
        }).get().join(',');
        ...
        $.ajax({
            async: false,
            type: "POST",
            url:'/admin/bookshelf/doDeleteObjects',
            data:{ids:text},
            contentType : "application/x-www-form-urlencoded; charset=utf-8",
            dataType: "text",
            complete:function () {
